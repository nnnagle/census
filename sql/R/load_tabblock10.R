...
system(sprintf('%spsql -U postgres -d tiger -c "DROP TABLE IF EXISTS tabblock10"', sql.loc))
...
for(f in zip.files){
  ...
  shp2pgsql.cmd <- sprintf('%sshp2pgsql %s -I -s 4269 -W "latin1" -D %s tabblock10 | %spsql -d tiger -U postgres ' , 
